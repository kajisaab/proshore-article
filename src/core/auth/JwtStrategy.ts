...
import { JwtInterface } from '@config/index';

export function createToken(payload: string | object | Buffer, jwtConfig: JwtInterface, audience: string): string {
...
export async function verifyToken(token: string, jwtConfig: JwtInterface): Promise<jwt.JwtPayload> {
